...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Ajouter des articles</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                           Ajouter les informations sur un article
                        </div>
                        <div class="panel-body">
                            <div class="row">
								...
								extract($_POST);
								if(isset($ok)){
									$shop="../../images/shop/".$_FILES['shop']['name'];
									move_uploaded_file($_FILES['shop']['tmp_name'],$shop);
									$large="../../images/large/".$_FILES['large']['name'];
									move_uploaded_file($_FILES['large']['tmp_name'],$large);
									$small="../../images/small/".$_FILES['small']['name'];
									move_uploaded_file($_FILES['small']['tmp_name'],$small);
								}
								?>
                                    <form role="form" action ="<?php echo $_SERVER['PHP_SELF']?>" method="POST" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label>CODE articles</label>
                                            <input name="acode" class="form-control">
                                        </div>
										<div class="form-group">
                                            <label>CODE CAT</label>
                                            <select  name="acodec"class="form-control">
											<option>1</option><option>2</option><option>3</option><option>4</option><option>5</option>
											<option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>
											</select>
                                        </div>
										<div class="form-group">
                                            <label>LIBELLE articles</label>
                                            <input name="alibart" class="form-control">
                                        </div>
										<div class="form-group">
                                            <label>PRIX</label>
                                            <input name="aprix" class="form-control">
                                        </div>
										<div class="form-group">
                                            <label>IMAGE</label>
                                            <input name="aimage" class="form-control">
                                        </div>
										<div class="form-group">
                                            <label>File image shop</label>
                                            <input name="shop" type="file">
											<label>File image large</label>
											<input name="large" type="file">
											<label>File image small</label>
											<input name="small" type="file">
                                        </div>
										<div class="form-group">
                                            <label>DESCRIPTION</label>
                                            <input  name="aDESCRIPTION"class="form-control">
                                        </div>
										<div class="form-group">
                                            <label>QTE</label>
                                            <input name="aQTE" class="form-control">
                                        </div>
									...
									$qte=$_POST['aQTE'];
									$sql=mysql_query("insert into article values('$code','$codec','$lib','','',$prix,'$image','$desc','$qte')");
									if($sql)
									{echo "bien ajouter";}else{echo "non ajouter";}
									}
									?>
                                </div>
                            </div>
